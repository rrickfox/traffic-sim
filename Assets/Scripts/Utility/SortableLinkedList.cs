...
namespace Utility
{
        ...
        where TNode : class, ISortableListNode
    {
        ...
        // Runtime: O(1)
        private static void InsertBefore(TNode before, TNode toInsert)
        {
            var after = before.previous;
            if (after != null)
            {
                after.next = toInsert;
        ...
        public IEnumerable<TNode> AllGreater(TNode node)
        {
            ...
            foreach (var otherNode in LookForward(node))
            {
                ...
                {
                    restIsGreater = _comparer.Compare(node, otherNode) < 0;
                    if (restIsGreater)
                        yield return otherNode;
                }
            }
